...
//    NSString *strEventIdentifier = self.appDelegate.eventManager.selectedEventIdentifier;
    ...
    if(self.strDate){
        self.eventStartDate = [self.appDelegate.eventManager getDateFromString:self.strDate];
        self.eventEndDate = [self.appDelegate.eventManager getDateFromString:self.strDate];
    }
    
    if(self.strLocationInfo){
        self.eventTitle =[NSString stringWithFormat:@"-%@", self.strLocationInfo];
    }
...
-(void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath{
    if ((indexPath.section == 0 && (indexPath.row == 1 || indexPath.row == 2)) ||
        ...
        self.eventTitle = self.textField.text;
...
//    // Set its calendar.
//    NSString *calendarIdentifier = self.appDelegate.eventManager.selectedCalendarIdentifier;
//    event.calendar = [self.appDelegate.eventManager.eventStore calendarWithIdentifier:calendarIdentifier];
...
//        [self.navigationController popViewControllerAnimated:YES];
    ...
    self.appDelegate.eventManager.currentEvent = event;
    
    if(self.mode == 0){
        [self performSegueWithIdentifier:@"unwindSegueToNewPushFromCalendar" sender:self];
    }else{
        [self.navigationController popViewControllerAnimated:YES];
    }
