...
- (void)viewDidLoad {
    ...
    NSData *photoData = [self.anotherUser  objectForKey:keyPhoto];
    UIImage *photoImg = [UIImage imageWithData:photoData];
    [self.btnPhoto setImage:photoImg forState:UIControlStateNormal];
    
    self.usernameTextField.text = [self.anotherUser valueForKey:kpRealUsername];
    
    NSString* bioText = [self.anotherUser valueForKey:keyBio];
...
- (BOOL) checkExisting{
    ...
    [query whereKey:keyOtherUser equalTo:self.anotherUser];
